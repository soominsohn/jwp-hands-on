...
class Stage0Test {
    ...
    /**
     * DriverManager JDBC 드라이버를 관리하는 가장 기본적인 방법.
    ...
    /**
     * DataSource 데이터베이스, 파일 같은 물리적 데이터 소스에 연결할 때 사용하는 인터페이스.
     * 구현체는 각 vendor에서 제공한다.
     * 테스트 코드의 JdbcDataSource 클래스는 h2에서 제공하는 클래스다.
     * <p>
     * DirverManager가 아닌 DataSource를 사용하는 이유 - 애플리케이션 코드를 직접 수정하지 않고 properties로 디비 연결을 변경할 수 있다.
